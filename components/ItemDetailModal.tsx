...
export default function ItemDetailModal({ item, onClose, onEdit }: ItemDetailModalProps) {
  ...
  const shareOnWhatsApp = () => {
    ...
    if (item.type === 'password') {
      const p = item as PasswordItem;
      text += `User: ${p.fields.username}\nPass: ${p.fields.password}`;
    } else if (item.type === 'card') {
      const c = item as CardItem;
      text += `No: ${c.fields.cardNumber}\nExp: ${c.fields.expiry}\nCVV: ${c.fields.cvv}`;
    }
  ...
  return (
    <Modal
      ...
      footer={
        <div style={{ display: 'flex', gap: 8, width: '100%' }}>
          <button className="btn btn-ghost" onClick={() => setShowMove(!showMove)} title="Move to folder">
            <Move size={14} /> Move
          </button>
          <button className="btn btn-ghost" onClick={shareOnWhatsApp} title="Share on WhatsApp">
    ...
    >
      {showMove && (
        <div className="glass-card" style={{ padding: 12, marginBottom: 16, border: '1px solid var(--accent-primary)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <span style={{ fontSize: 13, fontWeight: 600 }}>Move to Folder</span>
            <button className="action-btn" onClick={() => setShowMove(false)}><X size={14} /></button>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <button className={`btn btn-ghost ${item.folderId === null ? 'bg-primary/10' : ''}`} style={{ fontSize: 12, padding: '6px 8px' }} onClick={() => handleMove(null)}>
              No Folder
            </button>
            {folders.map(f => (
              <button 
                ...
                style={{ fontSize: 12, padding: '6px 8px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} 
