...
import {
  LayoutDashboard, KeyRound, CreditCard, FileText, ScanLine,
  FolderOpen, Users, Lock, BookOpen, Download,
} from 'lucide-react';
...
const GUIDES: {
  ...
  },
  {
    ...
    body: 'In Chrome: Settings → Autofill and passwords → Google Password Manager → Settings (gear) → Export passwords. Choose the CSV file here in CredsHub. Your CSV is only processed in the browser for the import request.',
  },
  {
    icon: CreditCard,
    title: 'Cards',
...
export default function HelpGuideModal({ open, onClose }: HelpGuideModalProps) {
  return (
    <Modal open={open} onClose={onClose} title="How to use CredsHub" maxWidth={560}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxHeight: 'min(62vh, 520px)', overflowY: 'auto', paddingRight: 4 }}>
        {GUIDES.map(({ icon: Icon, title, body }) => (
          <div
            key={title}
            className="glass-card"
            style={{ padding: 14, display: 'flex', gap: 14, alignItems: 'flex-start', borderRadius: 'var(--radius-md)' }}
          >
            <div
              style={{
                width: 40,
      ...
      <div style={{ marginTop: 16, fontSize: 12, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 8 }}>
        <BookOpen size={14} /> Open anytime from the sidebar: <strong style={{ color: 'var(--text-secondary)' }}>How to use</strong>
      </div>
    </Modal>
  );
}
